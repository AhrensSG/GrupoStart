...
export default function ContactTable({ contacts, userId, onDelete, onUpdate, onPin }) {
  ...
  const saveRound = useCallback(async (contactId, roundIndex, field, value) => {
    ...
    try {
      ...
      const targetLen = Math.max(contact.contactos.length, roundIndex + 1)
      const padded = targetLen > contact.contactos.length
        ? [...contact.contactos, ...Array.from({ length: targetLen - contact.contactos.length }, () => ({ clasificacion: "", fecha: "", estado: "", hora_proximo_contacto: "" }))]
        : contact.contactos
      const updatedContactos = padded.map((r, i) => {
          ...
          <div key={id} className="group relative">
            ...
            {isExpanded && (
              <div className="px-4 sm:px-6 pb-4 sm:pb-5">
                {id && (<div className="mb-3 p-2 sm:p-3 bg-gray-50 rounded-xl"><div className="flex items-center gap-2 mb-2"><svg className="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg><span className="text-xs font-semibold text-gray-500">Datos de contacto</span></div><div className="flex flex-col sm:flex-row gap-2 sm:gap-3"><input type="text" value={(contactEdits[`nombre-${id}`] ?? c.nombre) || ""} onChange={(e) => { const val = e.target.value; setContactEdits((prev) => ({ ...prev, [`nombre-${id}`]: val })) }} onBlur={(e) => saveContactField(id, "nombre", e.target.value)} placeholder="Nombre completo" className="w-full sm:flex-1 px-2.5 py-1.5 rounded-lg text-xs border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-[#0051FF]/20 focus:border-[#0051FF] placeholder:text-gray-300" /><input type="text" value={(contactEdits[`celular-${id}`] ?? c.celular) || ""} onChange={(e) => { const val = e.target.value; setContactEdits((prev) => ({ ...prev, [`celular-${id}`]: val })) }} onBlur={(e) => saveContactField(id, "celular", e.target.value)} placeholder="Celular" className="w-full sm:flex-1 px-2.5 py-1.5 rounded-lg text-xs border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-[#0051FF]/20 focus:border-[#0051FF] placeholder:text-gray-300" /><input type="text" value={(contactEdits[`email-${id}`] ?? c.email) || ""} onChange={(e) => { const val = e.target.value; setContactEdits((prev) => ({ ...prev, [`email-${id}`]: val })) }} onBlur={(e) => saveContactField(id, "email", e.target.value)} placeholder="Email" className="w-full sm:flex-1 px-2.5 py-1.5 rounded-lg text-xs border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-[#0051FF]/20 focus:border-[#0051FF] placeholder:text-gray-300" /><select value={contact.red_social || ""} onChange={(e) => { const val = e.target.value; setContactEdits((prev) => ({ ...prev, [`red_social-${id}`]: val })); saveContactField(id, "red_social", val) }} className="w-full sm:flex-1 px-2.5 py-1.5 rounded-lg text-xs border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-[#0051FF]/20 focus:border-[#0051FF]">{REDES_SOCIALES.map((opt) => (<option key={opt} value={opt}>{opt || "Sin red social"}</option>))}</select><input type="text" value={(contactEdits[`nombre_usuario-${id}`] ?? contact.nombre_usuario) || ""} onChange={(e) => { const val = e.target.value; setContactEdits((prev) => ({ ...prev, [`nombre_usuario-${id}`]: val })) }} onBlur={(e) => saveContactField(id, "nombre_usuario", e.target.value)} placeholder="Nombre de usuario" disabled={!contact.red_social || contact.red_social === "WhatsApp"} className={`w-full sm:flex-1 px-2.5 py-1.5 rounded-lg text-xs border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-[#0051FF]/20 focus:border-[#0051FF] placeholder:text-gray-300 ${!contact.red_social || contact.red_social === "WhatsApp" ? "bg-gray-100 text-gray-400 cursor-not-allowed" : ""}`}/></div></div>)}
                ...
                <div className="space-y-1">
                  {Array.from({ length: 5 }, (_, j) => {
                    const r = c.contactos[j] || { clasificacion: "", fecha: "", estado: "", hora_proximo_contacto: "" }
                    const prevRound = c.contactos[j - 1]
                    const prevHasData = j === 0 || (prevRound && (prevRound.clasificacion && prevRound.clasificacion !== "Pendiente"))
                    const block = !prevHasData
                    ...
                    return (
                      <div key={j} className={`grid grid-cols-1 md:grid-cols-[16px_32px_2fr_1fr_150px_100px_16px] gap-1.5 sm:gap-2 px-2 sm:px-3 py-2 sm:py-2.5 bg-gray-50 rounded-xl items-start md:items-center ${block ? "opacity-40" : ""}`}>
                        ...
                        <div><label className="md:hidden text-[9px] font-semibold text-gray-400 uppercase mb-0.5 block">Clasificación</label>{id ? (<div className="flex gap-1"><select value={r.clasificacion} disabled={block} onChange={(e) => saveRound(id, j, "clasificacion", e.target.value)} className="flex-1 min-w-0 px-2 py-1.5 rounded-lg text-xs border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-[#0051FF]/20 focus:border-[#0051FF] disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed">{CLASIFICACIONES.map((opt) => (<option key={opt} value={opt}>{opt || "Sin clasificación"}</option>))}</select>{r.clasificacion === "No interesado" && (<select value={r.estado} onChange={(e) => saveRound(id, j, "estado", e.target.value)} className="flex-1 min-w-0 px-2 py-1.5 rounded-lg text-xs border border-red-200 bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-400"><option value="">Motivo</option>{NO_INTERESADO_REASONS.map((motivo) => (<option key={motivo} value={motivo}>{motivo.replace("No interesado: ", "")}</option>))}</select>)}</div>) : (r.clasificacion && <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${getClasificacionColor(r.clasificacion)}`}>{r.clasificacion}</span>)}</div>
                        <div><label className="md:hidden text-[9px] font-semibold text-gray-400 uppercase mb-0.5 block">Notas</label>{id ? (<input type="text" value={r.clasificacion === "No interesado" ? "" : estadoValue} disabled={block} onChange={(e) => { if (r.clasificacion !== "No interesado") handleEstadoChange(id, j, e.target.value) }} onBlur={(e) => { if (r.clasificacion !== "No interesado") handleEstadoBlur(id, j, e.target.value) }} placeholder={r.clasificacion === "No hubo respuesta" ? "Sin respuesta" : r.clasificacion === "No interesado" ? "" : "Agregar nota..."} className={`w-full px-2 py-1.5 rounded-lg text-xs border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0051FF]/20 focus:border-[#0051FF] placeholder:text-gray-300 ${r.clasificacion === "No interesado" ? "bg-gray-100 text-gray-300 cursor-not-allowed" : "bg-white"} ${r.clasificacion ? getEstadoStyle(r.clasificacion) : ""} disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed`} readOnly={r.clasificacion === "No interesado"} />) : (r.estado && <span className={`inline-block px-2 py-1 rounded-md text-xs border ${getEstadoStyle(r.clasificacion)}`}>{r.estado.length > 30 ? r.estado.slice(0, 30) + "…" : r.estado}</span>)}</div>
                        <div><label className="md:hidden text-[9px] font-semibold text-gray-400 uppercase mb-0.5 block">Próx. contacto</label>{id ? (<input type="date" value={proxFecha ? proxFecha.split("/").reverse().join("-") : ""} disabled={block} onChange={(e) => { if (!e.target.value) return; const parts = e.target.value.split("-"); const fechaStr = `${parts[2]}/${parts[1]}/${parts[0]}`; handleProxFechaChange(id, j, fechaStr) }} onBlur={(e) => { if (!e.target.value) return; const parts = e.target.value.split("-"); const fechaStr = `${parts[2]}/${parts[1]}/${parts[0]}`; handleProxFechaBlur(id, j, fechaStr) }} className="w-full px-2 py-1.5 rounded-lg text-xs border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-[#0051FF]/20 focus:border-[#0051FF] disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed" title="Próximo contacto (manual)"/>) : (r.proxima_fecha || calcProximaFechaLocal(r.clasificacion, c.contactos[0]?.fecha || "")) ? (<span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] font-semibold bg-orange-50 text-orange-700 border border-orange-200"><span className="text-orange-400 font-normal">Próx.:</span> {r.proxima_fecha || calcProximaFechaLocal(r.clasificacion, c.contactos[0]?.fecha || "")}</span>) : (<span className="text-xs text-gray-300">—</span>)}</div>
                        <div><label className="md:hidden text-[9px] font-semibold text-gray-400 uppercase mb-0.5 block">Hora</label>{j > 0 && id ? (<input type="time" value={r.hora_proximo_contacto || ""} disabled={block} onChange={(e) => saveRound(id, j, "hora_proximo_contacto", e.target.value)} className="w-full px-2 py-1.5 rounded-lg text-xs border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-[#0051FF]/20 focus:border-[#0051FF] disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed" />) : (<span className="text-xs text-gray-300">—</span>)}</div>

  ...
  const saveRound = useCallback(async (contactId, roundIndex, field, value) => {
    ...
    try {
      ...
      if (field === "clasificacion" && value === "Pendiente") {
        const hasLater = contact.contactos.slice(roundIndex + 1).some((r) => r.clasificacion && r.clasificacion !== "Pendiente")
        if (hasLater) return
      }
            ...
            {isExpanded && (
              <div className="px-4 sm:px-6 pb-4 sm:pb-5">
                ...
                <div className="space-y-1">
                  {(function() { const firstIdx = getFirstDataIndex(c.contactos); const count = Math.min(5 - firstIdx, 5); return Array.from({ length: count }, (_, j) => {
                    const idx = firstIdx + j
                    const r = c.contactos[idx] || { clasificacion: "", fecha: "", estado: "", hora_proximo_contacto: "" }
                    ...
                    return (
                      <div key={idx} className="grid grid-cols-1 md:grid-cols-[16px_32px_2fr_1fr_150px_100px_16px] gap-1.5 sm:gap-2 px-2 sm:px-3 py-2 sm:py-2.5 bg-gray-50 rounded-xl items-start md:items-center">
                        ...
                        <div><label className="md:hidden text-[9px] font-semibold text-gray-400 uppercase mb-0.5 block">Clasificación</label>{id ? (<div className="flex gap-1"><select value={currentClasif} disabled={c.contactos.slice(idx + 1).some((r) => r.clasificacion && r.clasificacion !== "Pendiente")} onChange={(e) => { if (e.target.value === "Pendiente" && c.contactos.slice(idx + 1).some((r) => r.clasificacion && r.clasificacion !== "Pendiente")) return; setClasifEdits((prev) => ({ ...prev, [clasifKey]: e.target.value })); saveRound(id, idx, "clasificacion", e.target.value) }} className="flex-1 min-w-0 px-2 py-1.5 rounded-lg text-xs border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-[#0051FF]/20 focus:border-[#0051FF] disabled:bg-gray-100 disabled:cursor-not-allowed">{CLASIFICACIONES.map((opt) => (<option key={opt} value={opt}>{opt || "Sin clasificación"}</option>))}</select>{currentClasif === "No interesado" && (<select value={r.estado} onChange={(e) => saveRound(id, idx, "estado", e.target.value)} className="flex-1 min-w-0 px-2 py-1.5 rounded-lg text-xs border border-red-200 bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-400"><option value="">Motivo</option>{NO_INTERESADO_REASONS.map((motivo) => (<option key={motivo} value={motivo}>{motivo.replace("No interesado: ", "")}</option>))}</select>)}</div>) : (r.clasificacion && <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${getClasificacionColor(r.clasificacion)}`}>{r.clasificacion}</span>)}</div>

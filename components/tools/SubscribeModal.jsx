...
export default function SubscribeModal({ user, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.init_point) {
        localStorage.setItem("pending_preapproval_" + user.id, JSON.stringify({
          id: data.id,
          email: mpEmail.trim(),
          timestamp: Date.now()
        }))
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl max-w-md w-full mx-4 p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <a
            href="https://wa.me/543704619402?text=Hola%20GrupoStart%2C%20tengo%20una%20consulta%20sobre%20la%20suscripci%C3%B3n%20al%20Sistema%20de%20Seguimiento%20de%20Leads"
            target="_blank"
            rel="noopener noreferrer"
            className="mt-4 flex items-center justify-center gap-2 text-sm text-gray-500 hover:text-green-600 transition-colors"
          >

              ...
              {[rounds[selectedRound]].map((r, idx) => {
                ...
                return (
                  <div key={i} ref={(el) => { roundRefs.current[i] = el }} className="grid grid-cols-1 md:grid-cols-[80px_3fr_1fr_120px_85px] gap-2 p-3 bg-gray-50 rounded-xl items-start">
                    ...
                    <div className="flex gap-1.5"><select value={r.clasificacion} onChange={(e) => updateRound(i, "clasificacion", e.target.value)} disabled={rounds.slice(i + 1).some((r2) => r2.clasificacion && r2.clasificacion !== "Pendiente")} className="flex-1 px-2.5 py-2 border border-gray-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#0051FF]/20 focus:border-[#0051FF] disabled:bg-gray-100 disabled:cursor-not-allowed"><option value="">Clasificación</option>{CLASIFICACIONES.map((c) => (<option key={c} value={c}>{c}</option>))}</select><select value={r.estado} onChange={(e) => updateRound(i, "estado", e.target.value)} className={`flex-1 px-2.5 py-2 border border-red-200 rounded-lg text-sm bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-400 ${r.clasificacion !== "No interesado" ? "hidden" : ""}`} style={{ display: r.clasificacion === "No interesado" ? "block" : "none" }}><option value="">Motivo</option>{NO_INTERESADO_REASONS.map((motivo) => (<option key={motivo} value={motivo}>{motivo.replace("No interesado: ", "")}</option>))}</select></div>

...
// Meta borra los medios subidos a los 30 días: re-subimos antes para que no expire.
...
const TEMPLATE_NAMES = {
  ...
  derivacion: process.env.WHATSAPP_TEMPLATE_DERIVACION || "gs_derivacion_ia",
}
...
export async function sendButtonMessage(phone, body, buttons) {
  ...
  const safeButtons = (buttons || []).slice(0, 3).map((button, index) => ({
    type: "reply",
    reply: {
      id: String(button.id || `btn_${index}`).slice(0, 200),
      title: String(button.title || "Opción").slice(0, 20),
    },
  }))
  if (!safeButtons.length) return sendTextViaWhatsApp(phone, body)
  try {
    const res = await fetch(`https://graph.facebook.com/v22.0/${WHATSAPP_PHONE_ID}/messages`, {
      method: "POST",
      ...
      body: JSON.stringify({
        ...
        to: normalizePhone(phone),
        type: "interactive",
        interactive: { type: "button", body: { text: body.slice(0, 1024) }, action: { buttons: safeButtons } },
      }),
    })
...
/**
 * Envía la plantilla de video de presentación. Las plantillas no dependen de la
 * ventana de atención de 24 hs, así que el video llega siempre y además deja abierta
 * esa ventana para los mensajes libres que siguen (respuesta de la IA y nodo 1).
 */
export async function sendVideoTemplate(phone) {
  const name = process.env.WHATSAPP_TEMPLATE_VIDEO || "video"
  if (!WHATSAPP_TOKEN || !WHATSAPP_PHONE_ID || !name) {
    console.warn("[WhatsApp Cloud] Token, Phone ID o WHATSAPP_TEMPLATE_VIDEO no configurados, saltando el video a", phone)
    return false
  }
  try {
    const res = await fetch(`https://graph.facebook.com/v22.0/${WHATSAPP_PHONE_ID}/messages`, {
      ...
      body: JSON.stringify({
        ...
        to: normalizePhone(phone),
        type: "template",
        template: { name, language: { code: TEMPLATE_LANGUAGE } },
      }),
    })
    if (!res.ok) {
      const err = await res.json().catch(() => ({}))
      console.error("[WhatsApp Cloud] Plantilla de video:", err.error?.message || res.statusText)
      return false
    }
    return true
  } catch (err) {
    console.error("[WhatsApp Cloud] Video template error:", err.message)
    return false
  }
}
...
export async function sendListMessage(phone, body, buttonText, options) {
  if (!WHATSAPP_TOKEN || !WHATSAPP_PHONE_ID) return false
  const rows = (options || []).slice(0, 10).map((option, index) => ({
    id: String(option.id || `row_${index}`).slice(0, 200),
    title: String(option.title || "Opción").slice(0, 24),
    ...(option.description ? { description: String(option.description).slice(0, 72) } : {}),
  }))
  if (!rows.length) return sendTextViaWhatsApp(phone, body)
  try {
    const res = await fetch(`https://graph.facebook.com/v22.0/${WHATSAPP_PHONE_ID}/messages`, {
      method: "POST",
      headers: { Authorization: `Bearer ${WHATSAPP_TOKEN}`, "Content-Type": "application/json" },
      body: JSON.stringify({
        ...
        interactive: {
          ...
          action: { button: String(buttonText || "Ver opciones").slice(0, 20), sections: [{ title: "Opciones", rows }] },
        },
      }),
    })
    if (!res.ok) return false
...
/** Avisa a un número interno que la IA no pudo responder una consulta. */
export async function sendHandoffNotification(phone, { numero, consulta } = {}) {
  const msg =
    `Hola soy Sofi IA, el número ${numero || "—"} consultó ${consulta || "—"}. ` +
    `Busqué en mis conocimientos y no logré encontrar la respuesta a esa consulta. ` +
    `¿Me podrías ayudar? El usuario espera atención humana.`
  return sendWithTemplate(phone, TEMPLATE_NAMES.derivacion, [numero || "—", consulta || "—"], msg)
}

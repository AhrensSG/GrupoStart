...
export async function getWaAiState(phone) {
  await ensureSetup()
  const client = await pool.connect()
  try {
    const { rows } = await client.query("SELECT ai_state FROM wa_conversations WHERE phone = $1", [normalizeWaPhone(phone)])
    const raw = rows[0]?.ai_state
    if (!raw) return {}
...
export async function saveWaAiState(phone, state) {
  await ensureSetup()
  const phoneNorm = normalizeWaPhone(phone)
  if (!phoneNorm) return null
  const client = await pool.connect()
  try {
    await client.query(
      `INSERT INTO wa_conversations (phone, ai_state) VALUES ($1, $2::jsonb)
       ON CONFLICT (phone) DO UPDATE SET ai_state = $2::jsonb, updated_at = NOW()`,
      [phoneNorm, JSON.stringify(state || {})]
    )
    return phoneNorm
  } finally {
    client.release()
...
export async function getWaMediaCache(key) {
  await ensureSetup()
  const client = await pool.connect()
  try {
    const { rows } = await client.query(
      "SELECT media_id, mime_type, bytes, uploaded_at FROM wa_media_cache WHERE key = $1",
      [key]
    )
    return rows[0] || null
  } finally {
...
export async function saveWaMediaCache(key, { mediaId, mimeType, bytes }) {
  await ensureSetup()
  const client = await pool.connect()
  try {
    await client.query(
      `INSERT INTO wa_media_cache (key, media_id, mime_type, bytes, uploaded_at)
       VALUES ($1, $2, $3, $4, NOW())

...
function calcProximaFecha(clasificacion, _estado, fechaBase) {
  ...
  const days = daysMap[clasificacion]
...
export async function getAllContacts() {
  const client = await pool.connect()
  try {
    const contacts = (await client.query("SELECT * FROM contacts ORDER BY nombre ASC")).rows
...
export async function updateUserProfile(data) {
  ...
  try {
    ...
    if (!current) {
      await client.query(
        "INSERT INTO user_profile (hora_ingreso, hora_salida, whatsapp_api_url, whatsapp_api_token) VALUES ($1, $2, $3, $4)",
        [data.hora_ingreso || "09:00", data.hora_salida || "18:00", data.whatsapp_api_url || "", data.whatsapp_api_token || ""]
      )
      return
    }
    const sets = []

...
export async function getAllContacts(userId) {
  ...
  const client = await pool.connect()
  try {
    const contacts = (await client.query("SELECT * FROM contacts WHERE user_id = $1 ORDER BY pinned DESC, nombre ASC", [userId])).rows
    const rounds = (await client.query("SELECT cr.* FROM contact_rounds cr INNER JOIN contacts c ON c.id = cr.contact_id WHERE c.user_id = $1 ORDER BY cr.contact_id, cr.round_index ASC", [userId])).rows
...
export async function updateUserProfile(data, userId) {
  ...
  try {
    ...
    if (!current) {
      await client.query(
        "INSERT INTO user_profile (hora_ingreso, hora_salida, whatsapp_api_url, whatsapp_api_token, company_name, company_logo, user_id) VALUES ($1, $2, $3, $4, $5, $6, $7)",
        [data.hora_ingreso || "09:00", data.hora_salida || "18:00", data.whatsapp_api_url || "", data.whatsapp_api_token || "", data.company_name || "", data.company_logo || "", userId]
      )
      return
    }
    const sets = []
      ...
      for (let i = 0; i < contactos.length; i++) {
        ...
        if (r.clasificacion !== undefined || r.fecha !== undefined || r.estado !== undefined || r.hora_proximo_contacto !== undefined || r.proxima_fecha !== undefined) {
          ...
          if (roundSets.length > 0) {
            ...
            const roundIdx = ridx
            if (ownsContact) {
              await client.query(`UPDATE contact_rounds SET ${roundSets.join(", ")} WHERE contact_id = $${roundIdx} AND round_index = $${roundIdx + 1}`, roundValues)
            }
          }
        }
      }

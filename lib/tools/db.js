...
async function setupDatabase() {
  try {
    ...
    try {
      ...
      await client.query("ALTER TABLE user_profile ADD COLUMN IF NOT EXISTS user_phone VARCHAR(100) DEFAULT ''")
      await client.query("ALTER TABLE contact_rounds ADD COLUMN IF NOT EXISTS upcoming_notified_date VARCHAR(20) DEFAULT ''")
      await client.query("ALTER TABLE contact_rounds ADD COLUMN IF NOT EXISTS due_notified_date VARCHAR(20) DEFAULT ''")
      await client.query("ALTER TABLE contact_rounds ADD COLUMN IF NOT EXISTS overdue_notified_date VARCHAR(20) DEFAULT ''")
      ...
      `)
      await client.query("ALTER TABLE subscriptions ADD COLUMN IF NOT EXISTS preapproval_id VARCHAR(255) DEFAULT ''")
      await client.query("CREATE INDEX IF NOT EXISTS idx_contacts_user_id ON contacts(user_id)")
      await client.query("CREATE INDEX IF NOT EXISTS idx_contacts_deleted_at ON contacts(deleted_at)")
      await client.query("CREATE INDEX IF NOT EXISTS idx_subscriptions_uid ON subscriptions(uid)")
...
export async function updateUserProfile(data, userId) {
  ...
  try {
    ...
    if (!current) {
      await client.query(
        "INSERT INTO user_profile (hora_ingreso, hora_salida, horario_ranges, whatsapp_api_url, whatsapp_api_token, company_name, company_logo, user_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
        [data.hora_ingreso || "09:00", data.hora_salida || "18:00", data.horario_ranges || "", data.whatsapp_api_url || "", data.whatsapp_api_token || "", data.company_name || "", data.company_logo || "", userId]
      )
      return
    }
    const sets = []
...
export async function setUserSubscribed(uid, email, preapproval_id) {
  ...
  try {
    ...
    if (rows.length > 0) {
      await client.query("UPDATE subscriptions SET status = 'active', preapproval_id = $2, updated_at = NOW() WHERE uid = $1", [uid, preapproval_id || ""])
    } else {
      await client.query(
        "INSERT INTO subscriptions (uid, email, status, preapproval_id) VALUES ($1, $2, 'active', $3)",
...
export async function getAllActiveSubscribedUsers() {
  ...
  const client = await pool.connect()
  try {
    const { rows } = await client.query("SELECT uid, email FROM subscriptions WHERE status = 'active'")
    return rows
  } finally {
    client.release()
  }
       ...
       WHERE c.user_id = $1 AND c.deleted_at IS NULL
         ...
         AND c.id NOT IN (
           ...
           WHERE contact_id = c.id AND round_index > cr.round_index
             AND proxima_fecha >= $2 AND clasificacion != 'Pendiente'
         )`,

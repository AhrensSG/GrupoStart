...
async function setupDatabase() {
  try {
    ...
    try {
      ...
      await client.query("ALTER TABLE subscriptions ADD COLUMN IF NOT EXISTS expires_at TIMESTAMP DEFAULT NULL")
      ...
      `)
      await client.query("CREATE INDEX IF NOT EXISTS idx_wa_messages_phone ON wa_messages(conversation_phone)")
      await client.query("CREATE INDEX IF NOT EXISTS idx_wa_messages_created ON wa_messages(created_at)")
...
export async function saveWaOutgoingMessage({ to, body, waMessageId, status }) {
  await ensureSetup()
  ...
  try {
    ...
    await client.query(
      `INSERT INTO wa_messages (conversation_phone, direction, body, type, media_url, wa_message_id, status)
       VALUES ($1, 'out', $2, 'text', '', $3, $4)`,
      [phone, body || "", waMessageId || "", status || "sent"]
    )
    await client.query("COMMIT")
    return phone
  } catch (err) {

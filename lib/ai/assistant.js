...
import { getFlowQuestion, getFlowUi, getNextStage, isValidEmail, normalizeProfileUpdates, normalizeStage } from "./flow.js"
...
function completeReply(reply, ui, stage, outcome = null) {
  let text = String(reply || "").trim().replace(/(?:\s|,)*(?:si|sí)\s*(?:,|y)?\s*no\.?$/i, "").trim()
  if ((!outcome || outcome === "interesado") && stage !== "cierre" && !/[?¿]/.test(text)) {
...
export async function generateReply({ history = [], customerName = "", state = {}, now } = {}) {
  ...
  const requestedUi = sanitizeUi(parsed.ui)
  const ui = requestedUi ? getFlowUi(stageUpdate) : null
  ...
  if (action?.type === "meeting_request" && !isValidEmail(mergedProfile.email)) {
    return {
      reply: completeReply(parsed.reply, ui, stageUpdate),
      stageUpdate,
      profileUpdates,
      action: null,
      outcome: null,

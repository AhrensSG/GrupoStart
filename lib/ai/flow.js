...
import { PREGUNTA_COMENZAMOS } from "./motor-ventas.js"

const FLOWS = {
  inicio: { type: "text", question: "Hola, ¿en qué puedo ayudarte?" },
  // Entrada del bot: el cliente escribe y se decide si es una de las 4
  // preguntas predefinidas o una consulta libre para la IA.
  motor_ventas: { type: "text", question: "" },
  comenzamos: {
    question: PREGUNTA_COMENZAMOS,
    type: "buttons",
    options: [
      { id: "si", title: "Sí" },
      { id: "no", title: "No" },
  ...
  despedida: { type: "text", question: "" },
  derivado: { type: "text", question: "" },
  fin: { type: "text", question: "" },
  consentimiento: {
    question: "¿Comenzamos?",
    type: "buttons",
    options: [
...
export function getNextStage(stage, message) {
  ...
  if (stage === "comenzamos") {
    if (/^(no|nop|nel|no gracias|mejor no|no por ahora|paso|rechazar|dejemos)/.test(text)) return "despedida"
    // "sin falta" es una afirmación rioplatense ("¡por supuesto!"), no una negativa.
    if (/^(si|sí|dale|ok|claro|perfecto|de acuerdo|si por favor|si dale|empecemos|comenzar|adelante|quiero|va|sin falta)/.test(text)) return "fin"
    return null
  }
  if (stage === "despedida" || stage === "derivado" || stage === "fin") return null

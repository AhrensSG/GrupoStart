...
export function getNextStage(stage, message) {
  ...
  if (stage === "comenzamos") {
    if (/^(no|nop|nel|no gracias|mejor no|no por ahora|paso|rechazar|sin falta|dejemos)/.test(text)) return "despedida"
    if (/^(si|sí|dale|ok|claro|perfecto|de acuerdo|si por favor|si dale|empecemos|comenzar|adelante|quiero|va)/.test(text)) return "fin"
    return null
  }
